...
async function detectIntent(text, state, conversationContext = '') {
  ...
  const prompt = `
    You are an intent classifier and goal extractor for a fitness and wellness chatbot.
    ...
    Determine the user's intent and goals based on the message, conversation context, and state.
  ...
  const { intent, goal } = safeParseIntent(response);
