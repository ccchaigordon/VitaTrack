type InsightResponse = {
  summary: string[];
  nextFocus?: string | null;
  isFallback: boolean;
};

interface Props {
  insight: InsightResponse | null;
  // loading: boolean;
}

export function InsightsCard ({ insight, /*loading*/ }: Props) {
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm min-h-[160px] flex flex-col justify-center border border-gray-50">
      <div className="flex items-left gap-2 mb-3">
        <img src="./src/assets/Progress/Bot.svg" className="w-6 h-6 mt-[2px]" />
        <h3 className="text-xl font-bold text-gray-800">Weekly Insights</h3>
      </div>
      
      {/* {loading ? (
        <div className="flex items-center gap-2 text-gray-400 text-sm">
          <img src="src/assets/Loader.svg" className="animate-spin w-6 h-6" /> Generating analysis...
        </div>
      ) : */insight ? ( //}
        <div className="space-y-2 animate-in fade-in duration-500">
          <ul className="list-disc list-inside space-y-1 text-left">
            {insight.summary.map((line: string, i: number) => (
              <li key={i} className="text-sm text-gray-600 leading-relaxed">{line}</li>
            ))}
          </ul>
          {insight.nextFocus && (
            <div className="mt-3 bg-emerald-50 p-2 rounded-lg text-xs text-emerald-800 font-medium">
              🚀 Focus: {insight.nextFocus}
            </div>
          )}
          <p className="text-[9px] text-gray-300 mt-2 text-right">Non-medical guidance. Consult a doctor for health risks.</p>
        </div>
      ) : (
        <div className="text-sm text-gray-400 italic">
          Log your meals and workouts to receive personalized AI insights.
        </div>
      )}
    </div>
  );
};